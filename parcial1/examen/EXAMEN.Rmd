...
ggplot(trabajan, aes(x= Observaciones, y=Salario_mes, col=Categoria)) +  geom_line()
```


```{r, echo=TRUE,warning=FALSE,tidy=TRUE}
# Número de personas con padres separados por categoría
junior <- filter(datos, Edad <18)
senior <- filter(datos, Edad>=18 & Edad <=25)
mayor <- filter(datos, Edad > 25)
